...
| Notes + Tasks FABs (plain round CTAs, no badges) | `components/Fabs.tsx` | Hidden in Break + on completion screens (callers decide). Tasks FAB is idle-only: `showTasks={false}` during active sessions (Notes FAB stays). |
